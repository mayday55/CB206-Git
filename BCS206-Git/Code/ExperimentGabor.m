...
if isempty(GaborData.model_observer)
    ...
    %right_template = squeeze(grating(pi/4, 0.02));
    %left_template = squeeze(imagesc(grating(-pi/4, 0.02)));
    % left_template = squeeze(grating.genImages_grating(1, GaborData.stim_size, GaborData.stim_sp_freq_cpp, GaborData.stim_std_sp_freq_cpp, GaborData.left_category, 2, 0, 1)) * 64.0 + 127.0;
    left_template_noannulus = 64*squeeze(grating_no_annulus(-pi/4, 0, 1))+127;
    right_template_noannulus = 64*squeeze(grating_no_annulus(pi/4, 0, 1))+127;
    left_template_raw = squeeze(grating(-pi/4, 0, 1,1));
    right_template_raw = squeeze(grating(+pi/4, 0, 1,1));
    left_template = 64*left_template_raw+127;
    right_template = 64*right_template_raw+127;
    right_tex = Screen('MakeTexture', wPtr,right_template_noannulus);
    left_tex = Screen('MakeTexture', wPtr,left_template_noannulus);
    [h, w] = size(right_template);
    
    % Further timing setup
    GaborData.blank_duration = (1/settings.monitorFPS) * GaborData.blank_frames;
...
try
    while trial <= GaborData.trials_per_block * GaborData.blocks
        ...
        if mod(trial, GaborData.trials_per_block) == 1
            ...
            if trial ~= 1 && isempty(GaborData.model_observer) && ~seen_block_notification
                ...
                if isempty(GaborData.model_observer)
                    sounds(-1, 1.5);
                    DrawFormattedText(wPtr, ...
                        [sprintf('You have completed %d blocks. ', block) ...
        ...
        GaborData.ideal_frame_signals(trial, :) = ...
            getSignal_grating(image_array - 127, template_contrast * left_template_raw) - ...
            getSignal_grating(image_array - 127, template_contrast * right_template_raw);
        ...
        if isempty(GaborData.model_observer)
            ...
            [I, ~, quit] = trialStimuliGabor(GaborData, image_array, wPtr, settings);

function [image_properties, broke_fixation, quit] = trialStimuliGabor(Data, image_array, wPtr, settings)
...
left_patch_noannulus = squeeze(grating_no_annulus([Data.left_category], 0, 1)) * 64.0 + 127.0;
right_patch_noannulus = squeeze(grating_no_annulus([Data.right_category], 0, 1)) * 64.0 + 127.0;
...
noise_mask = squeeze(grating([0], 1, 1, 1)) * Data.contrast(Data.current_trial) * 2 + 127.0;
image_texture(end) = Screen('MakeTexture', wPtr, noise_mask);

show_left_patch = Screen('MakeTexture', wPtr, left_patch_noannulus);
show_right_patch = Screen('MakeTexture', wPtr, right_patch_noannulus);

...
if isempty(GaborData.model_observer)
    ...
    left_template = 64*squeeze(grating(-pi/4, 0, 1,1))+127;
    right_template = 64*squeeze(grating(pi/4, 0, 1,1))+127;
...
try
    while trial <= GaborData.trials_per_block * GaborData.blocks
        ...
        GaborData.ideal_frame_signals(trial, :) = ...
            getSignal_grating(image_array - 127, GaborData.left_category, max(GaborData.noise(trial), .04),left_template) - ...
            getSignal_grating(image_array - 127, GaborData.right_category, max(GaborData.noise(trial), .04), right_template);

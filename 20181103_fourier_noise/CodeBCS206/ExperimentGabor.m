...
lft = -1*rad2deg(GaborData.left_category);
rgt = -1*rad2deg(GaborData.right_category);
...
if isempty(GaborData.model_observer)
    ...
    cd(fullfile(directory, 'CodeBCS206')) % Set the current directory
...
if isempty(GaborData.model_observer)
    ...
    left_template_noannulus = 64*squeeze(grating_no_annulus(GaborData.left_category, 0, 1))+127;
    right_template_noannulus = 64*squeeze(grating_no_annulus(GaborData.right_category, 0, 1))+127;
    left_template_raw = squeeze(grating(GaborData.left_category, 0, 1,1));
    right_template_raw = squeeze(grating(GaborData.right_category, 0, 1,1));
    right_tex = Screen('MakeTexture', wPtr,right_template_noannulus);
    left_tex = Screen('MakeTexture', wPtr,left_template_noannulus);
    [h, w] = size(right_template_raw);
...
%             end
        ...
        GaborData.ideal_frame_signals(trial, :) = ...
            bpg.getSignal(image_array - 127, lft,  0.04) - ...
            bpg.getSignal(image_array - 127, rgt, 0.04);
...
%            GaborData.eye_tracker_points{trial} = eye_tracker_points;
        ...
        %% Accuracy & Feedback
        GaborData.accuracy(trial) = GaborData.choice(trial) == GaborData.correct_answer(trial);%GaborData.ideal_answer(trial);
        if isempty(GaborData.model_observer)
